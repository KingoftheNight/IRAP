...
import sys
import os
from Visual import visual_longcommand_raylinux
now_path = os.getcwd()

commands = sys.argv[1]
db_path = os.path.join(os.path.join(now_path, 'irap'), 'pdbaa')
...
def blast(file, database_path, out):
    if out not in os.listdir(os.path.split(out)[0]):
        command = visual_longcommand_raylinux(file, database_path, out)
