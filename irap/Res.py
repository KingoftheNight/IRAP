...
# 约化氨基酸
def res_reduce(data, tp):
    index = ivis.visual_create_aa()
    data = data[1:]
    euclidean_box = {}
    print('>>>>>>>>>>>>>>>>>')
    m = 0
    ...
    for y in reduce_list:
        result = "type " + str(tp) + " size " + str(len(y.split("-"))) + " " + y
...
# res main
def res(resp_id, tp=75, out=False):
    ...
    if len(request) == 0:
        ...
        return
    print("\n约化中...")
    final = res_reduce(request.split('\t'), tp)
    print("约化完成.")
    if out == False:    
        time.sleep(0.5)
        # 打印约化列表
