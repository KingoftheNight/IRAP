...
def read_extract_raabook(path1, path2, out, raa):
    pos_pssm = iload.load_reload_pssm(os.path.join(os.path.join(now_path, 'PSSMs'), path1))
    neg_pssm = iload.load_reload_pssm(os.path.join(os.path.join(now_path, 'PSSMs'), path2))
...
def read_grid_folder(folder):
    pssm_path = iload.load_reload_feature(os.path.join(now_path, folder))
